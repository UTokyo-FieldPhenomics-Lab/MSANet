...
class MyLoss(nn.Module):
    def __init__(self, base_loss_fn=F.mse_loss, attention_factor=1.0, ssim_factor=1.0):
    ...
    def forward(self, input, target):
        ...
        # ssim_loss = 1 - ssim(input, target, data_range=1, size_average=True)
        # 计算注意力权重。这里我们简单地使用target heatmap自身作为注意力指示
        # attention_weights = torch.clamp(target, 0, 1)

        # count_loss = F.l1_loss(input, target)
        # weighted_loss = self.base_loss_fn(input * attention_weights, target * attention_weights)
...
class trainer:
    ...
    def get_datasets(self):
        ...
        transform_train = A.Compose(
            [   
                ...
                #         p=0.5),
                A.RandomScale((-0.4, 0.4)),
                A.Downscale(scale_min=0.2, scale_max=0.5, p=0.5),
    ...
    def train_one_epoch(self):
        ...
        for img, label256, _ in self.dataloader_train:
            self.iter += 1
            ...
            del bx0, bx1, bx2, bx3, bx4, img, label256
    ...
    def visualization(self):
        ...
        nums_to_plot = self.args.batch_size if self.args.batch_size <= 4 else 4
        for i in range(nums_to_plot):
            ax = fig.add_subplot(2, nums_to_plot, i+1)
            ax.set_xticks([])
            ax.set_yticks([])
            ...
            # plot mask
            mask = torch.unsqueeze(label256[i], 1)
            mask = denorm(mask,
                          mean=(0.485, 0.456, 0.406),
                          std=(0.229, 0.224, 0.225))
            ax = fig.add_subplot(2, nums_to_plot, nums_to_plot+i+1)
            ax.set_xticks([])
            ax.set_yticks([])
            ax.imshow(mask.detach().squeeze().cpu().numpy(), cmap='gray')
    ...
    def evaluation(self):
        ...
        with torch.no_grad():
            for img, points, mask, target in self.dataloader_val:
                ...
                coordinates = peak_local_max(rst[..., 0], min_distance=10, threshold_abs=0.5)
